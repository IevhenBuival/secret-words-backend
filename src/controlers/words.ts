...
export const createWord: RequestHandler<unknown, unknown, ICreateWordBody, unknown> = async (req, res, next) => {
    ...
    try {
        ...
        let existNumerator = await counterModel.findOne({ name: 'words_' + language }).select('+numerator').exec();
...
export const updateWord: RequestHandler<IUpdateWordParams, unknown, IUpdateWordBody, unknown> = async (req, res, next) => {
...
export const deleteWord: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        if (thisUser?.rights !== 'admin')
            throw createHttpError(403, "You can't access this word");
